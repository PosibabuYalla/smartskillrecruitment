...
export default function ElectricalEnroll() {
  const [step, setStep] = useState(0);
  const [done, setDone] = useState(false);
  const [data, setData] = useState({
    name: "", email: "", phone: "", city: "", qualification: "",
    course: "", batch: "", mode: "offline", experience: "fresher",
  });

  const update = (k: string, v: string) => setData(p => ({...p, [k]: v}));

  const inputClass = "w-full px-4 py-3 rounded-xl text-white text-sm outline-none transition-all focus:border-amber-400";
      ...
      <section className="pt-32 pb-20 px-4">
        <div className="max-w-2xl mx-auto">
          <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} className="text-center mb-10">
            <h1 className="text-5xl font-bold font-rajdhani gradient-text-amber mb-3">Enroll Now</h1>
            <p className="text-gray-400">Start your electrical career journey in 3 simple steps</p>
          </motion.div>
          ...
          <div className="flex items-center justify-center gap-2 mb-10">
            {steps.map((s, i) => (
              <div key={i} className="flex items-center gap-2">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all ${i < step ? 'step-done' : i === step ? 'step-active' : 'step-inactive'}`}>
                  {i < step ? <CheckCircle size={16}/> : i + 1}
                </div>
                <span className={`text-xs hidden sm:block ${i === step ? 'text-amber-400' : 'text-gray-500'}`}>{s}</span>
              ...
              <motion.div key={step} initial={{opacity:0, x:30}} animate={{opacity:1, x:0}} exit={{opacity:0, x:-30}}
                ...
                {step === 0 && (
                  <div className="flex flex-col gap-4">
                    <h2 className="text-2xl font-bold font-rajdhani text-amber-400 mb-2">Personal Information</h2>
                    {[
                      { k: "name", label: "Full Name", type: "text", ph: "Your full name" },
                      { k: "email", label: "Email Address", type: "email", ph: "your@email.com" },
                      { k: "phone", label: "Phone Number", type: "tel", ph: "+91 XXXXX XXXXX" },
                    ...
                    <div>
                      ...
                      <select value={data.qualification} onChange={e => update("qualification", e.target.value)}
                        ...
                        <option>ITI</option><option>Diploma</option><option>B.E/B.Tech</option>
                      </select>
                    </div>
                  </div>
                )}
                ...
                {step === 1 && (
                  <div className="flex flex-col gap-4">
                    <h2 className="text-2xl font-bold font-rajdhani text-amber-400 mb-2">Course Selection</h2>
                    <div>
                      ...
                      <div className="flex gap-3">
                        {["offline", "online", "hybrid"].map(m => (
                          <button key={m} onClick={() => update("mode", m)}
                            ...
                            style={data.mode === m ? {background:'rgba(245,158,11,0.2)', border:'1px solid #F59E0B', color:'#F59E0B'} : {background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', color:'#9CA3AF'}}>
                            {m}
                          </button>
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex gap-3">
                        {["fresher", "experienced"].map(e => (
                          <button key={e} onClick={() => update("experience", e)}
                            className="flex-1 py-2.5 rounded-xl text-sm font-semibold capitalize transition-all"
                            style={data.experience === e ? {background:'rgba(245,158,11,0.2)', border:'1px solid #F59E0B', color:'#F59E0B'} : {background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', color:'#9CA3AF'}}>
                            {e}
                          </button>
                        ))}
                      </div>
                ...
                {step === 2 && (
                  <div>
                    <h2 className="text-2xl font-bold font-rajdhani text-amber-400 mb-6">Review Your Details</h2>
                ...
                <div className="flex gap-3 mt-8">
                  ...
                  <button
                    ...
                    className="flex-1 py-3 rounded-xl font-bold text-black transition-transform hover:scale-105"
                    style={{background:'linear-gradient(135deg, #F59E0B, #D97706)'}}>
                    {step < 2 ? "Continue" : "Submit Enrollment"}
                  </button>
                </div>
              </motion.div>
            </AnimatePresence>
